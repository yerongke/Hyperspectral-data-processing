...
switch Model.type
  case 'iPLS'
	...
	if isempty(Model.xaxislabels)
       area([a b],[actualaxis(3) actualaxis(3)]); % Negative areas
       area([a b],[actualaxis(4) actualaxis(4)]);
       plot(Model.rawX') % To overlay spectra on area plot
	else
       area([Model.xaxislabels(a) Model.xaxislabels(b)],[actualaxis(3) actualaxis(3)]); % Negative areas
       area([Model.xaxislabels(a) Model.xaxislabels(b)],[actualaxis(4) actualaxis(4)]);
       plot(Model.xaxislabels,Model.rawX) % To overlay spectra on area plot
	end
	hold off
  ...
  case {'PLS'}
	...
	if isempty(Model.xaxislabels)
       ...
       for i=1:max(size(a))
          area([a(i) b(i)],[actualaxis(3) actualaxis(3)]); % Negative areas
          area([a(i) b(i)],[actualaxis(4) actualaxis(4)]);
       end
       plot(Model.rawX') % To overlay spectra on area plot
	else
       if isfield(Model,'windowsize') % If oneModel is based on mwModel
  ...
  case {'PLSprediction'}
	if isempty(Model.CalModel.xaxislabels)
       ...
       plot(Model.CalModel.xaxislabels,Model.CalModel.rawX)
       xlabel('Wavelength')
	...
	if isempty(Model.CalModel.xaxislabels)
       ...
       plot(Model.CalModel.rawX') % To overlay spectra on area plot
	else
       if isfield(Model.CalModel,'windowsize') % If oneModel is based on mwModel
       ...
       for i=1:max(size(a))
		...
		plot(Model.CalModel.xaxislabels,Model.CalModel.rawX) % To overlay spectra on area plot
	end
	hold off
...
switch Model.type
  case 'iPLS'
	...
	s1 = sprintf('r = %0.4g',r(1,2));
	[RMSE,Bias]=rmbi(plotYref,plotYpred);

    if strcmp(lower(Model.val_method),'test')
        s2 = sprintf('RMSEP = %0.3g',RMSE);
    else
        s2 = sprintf('RMSECV = %0.3g',RMSE);        
    end
	s3 = sprintf('Bias = %0.3g',Bias);
	%legend(s1,s2,s3,2);
  case {'PLS'}
	if strcmp(Model.val_method,'test')
        plotYref=Model.rawY(Model.segments,y_variable);
